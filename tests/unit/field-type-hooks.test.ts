...
describe('field-type `references` (modeled-object membership)', () => {
  const cur = buildFieldTypeRegistry([
    { name: 'acme_currency', base: 'string', references: { object: 'currency' } },
  ]);

  const currency = { name: 'currency', fields: [{ name: 'code', type: 'string', primary: true }] };
  const invoice = objectDef({ name: 'ccy', type: 'acme_currency' });

  it('buildGraph accepts a valid object target', () => {
    const objects = new ObjectRegistry({ fieldTypes: cur });
    objects.register(currency, { fieldTypes: cur });
    objects.register(invoice, { fieldTypes: cur });
    expect(codeOf(() => objects.buildGraph())).toBeUndefined();
  });

  it('buildGraph rejects a missing target object', () => {
    const objects = new ObjectRegistry({ fieldTypes: cur });
    objects.register(invoice, { fieldTypes: cur });
    expect(codeOf(() => objects.buildGraph())).toBe('graph.references.target.missing');
  });

  it('buildGraph rejects a missing target column', () => {
    const withColumn = buildFieldTypeRegistry([
      { name: 'acme_currency', base: 'string', references: { object: 'currency', column: 'nope' } },
    ]);
    const objects = new ObjectRegistry({ fieldTypes: withColumn });
    objects.register(currency, { fieldTypes: withColumn });
    objects.register(invoice, { fieldTypes: withColumn });
    expect(codeOf(() => objects.buildGraph())).toBe('graph.references.column.missing');
  });

  it('the write path enforces membership (data.field.references)', async () => {
    const objects = new ObjectRegistry({ fieldTypes: cur });
    objects.register(currency, { fieldTypes: cur });
    objects.register(invoice, { fieldTypes: cur });
    objects.buildGraph();
    ...
    let caught: SchemaError | undefined;
    try {
      await validateRecord(invoice as never, { id: '1', ccy: 'USD' }, WRITE_MODES.CREATE, {
        pool: emptyPool as never,
        registry: objects,
        locale: 'en',
      });
    ...
    expect(caught?.code).toBe('data.field.references');

    const found = { query: async () => ({ rows: [{ ok: 1 }], rowCount: 1 }) };
    await validateRecord(invoice as never, { id: '1', ccy: 'USD' }, WRITE_MODES.CREATE, {
      pool: found as never,
      registry: objects,
      locale: 'en',
